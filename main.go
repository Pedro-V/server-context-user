...
import (
	"context"
	"fmt"
	"github.com/Pedro-V/server-context-user/identity"
...
// We get a user from the context, and pass it to the business logic.
...
func (c Controller) Logout(rw http.ResponseWriter, r *http.Request) {
	ctx := r.Context()
	// If the context hasn't been properly set, then we deny access.
...
func (l LogicImpl) BusinessLogic(ctx context.Context, user string, data string) (string, error) {
	return fmt.Sprintf("Hello %s, thank you for sending me %s\n", user, data), nil
}
